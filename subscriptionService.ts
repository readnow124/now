...
export interface Subscription {
  ...
  // ✅ ADDED: 'incomplete' statuses so TypeScript doesn't crash, but we treat them as "No Access"
  status: 'active' | 'expired' | 'cancelled' | 'past_due' | 'trialing' | 'incomplete' | 'incomplete_expired'; 
...
export class SubscriptionService {
  ...
  // ✅ 1. Initiate Trial / Payment via Edge Function
  // This uses the Secure Edge Function (No direct DB writes from client)
  static async initiateTrialWithCard(
    priceId: string, 
    ...
    if (!response.ok) {
      ...
      throw new Error(err.error || 'Failed to initiate trial');
    }

    return await response.json();
  }

  // ✅ 2. Cancel Subscription (Secure)
  ...
  // ✅ 3. SMART WAIT (Fixes Race Condition & Pending Payments)
  // This loop waits until the subscription is confirmed VALID.
  // If status is 'incomplete', it keeps waiting. It ONLY returns true when status becomes 'active' or 'trialing'.
  static async waitForSubscription(userId: string, attempts = 12): Promise<boolean> {
    for (let i = 0; i < attempts; i++) {
      console.log(`⏳ Verifying subscription status... Attempt ${i + 1}/${attempts}`);
      const { subscription, hasAccess } = await this.checkSubscriptionAccess(userId);
      
      // We check for ACTUAL ACCESS.
      // If status is 'incomplete', hasAccess will be false, so this loop continues waiting.
      if (subscription && hasAccess) {
        console.log('✅ Subscription confirmed active/trialing in DB!');
        return true;
      }
      
      // Wait 1 second before retry (Giving Stripe Webhook time to process)
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
    console.warn('⚠️ Subscription verification timed out or payment is still pending.');
    return false;
  }
  ...
  // ✅ 4. Check Access Logic (Strict Mode)
  static async checkSubscriptionAccess(userId: string): Promise<{
    ...
    try {
      ...
      return {
        hasAccess: false, // Default to false on error for security
        subscription: null,
  ...
  private static fallbackAccessCheck(subscription: Subscription | null) {
    ...
    // We explicitly exclude 'incomplete' here.
    // 'incomplete' means "Payment Processing" or "Payment Failed".
    // Therefore, hasAccess is FALSE until the webhook flips it to 'active'.
    const hasAccess = (
  ...
  static async getAllSubscriptions(): Promise<(Subscription & { user_email?: string; restaurant_name?: string; })[]> {
    try {
      ...
      })) as (Subscription & { user_email?: string; restaurant_name?: string; })[];
    }
  }
